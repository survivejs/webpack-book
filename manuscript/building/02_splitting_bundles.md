...
T> To invalidate the bundles properly, you will attach hashes to the generated bundles in the *Adding Hashes to Filenames* chapter.
...
Everything should work the same way as earlier. This time around, however, it is more convenient to work with the plugin. You still have access to its functionality as before, but with a smaller amount of code.
...
The build result should remain the same. This time, however, webpack will pull only dependencies that are used in the project, and you don’t have to maintain the list anymore.
...
* **Normal chunks** - Normal chunks **don’t** contain webpack runtime. Instead, these can be loaded dynamically while the application is running. A suitable wrapper (JSONP for example) is generated for these. You will generate a normal chunk in the next chapter as you set up code splitting.
...
The situation is better now compared to the earlier. Note how small `app` bundle compared to the `vendor` bundle. To benefit from this split, you will set up caching in the next part of this book in the *Adding Hashes to Filenames* chapter.
